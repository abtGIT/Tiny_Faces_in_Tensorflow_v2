...
class Model():
    ...
    def conv_block(self, bottom, name, shape, strides=[1,1,1,1], padding="SAME",
      ...
      conv = tf.nn.conv2d(input=bottom, filters=weight, strides=strides, padding=padding)
      if has_bias:
    ...
    def conv_trans_layer(self, bottom, name, shape, strides=[1,1,1,1], padding="SAME", has_bias=False):
      ...
      weight = self._weight_variable_on_cpu(name, shape)
      nb, h, w, nc = tf.split(tf.shape(input=bottom), num_or_size_splits=4)
    ...
    def tiny_face(self, image):
        ...
        Returns:
          a score tensor
        """
        img = tf.pad(tensor=image, paddings=[[0, 0], [3, 3], [3, 3], [0, 0]], mode="CONSTANT")
        conv = self.conv_block(img, 'conv1', shape=[7, 7, 3, 64], strides=[1, 2, 2, 1], padding="VALID", add_relu=True)
        pool1 = tf.nn.max_pool2d(input=conv, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME')
        ...
        bs, height, width = tf.split(tf.shape(input=score4), num_or_size_splits=4)[0:3]
        _size = tf.convert_to_tensor(value=[height[0], width[0]])
